...
export function registerInvoiceHandlers(): void {
  ...
  handle<CreateInvoicePayload, InvoiceWithDetail>('invoices:create', null, async ({ orderId, due_date }) => {
  ...
  handle<DeleteInvoicePayload, { deleted: boolean }>('invoices:delete', ['ADMIN', 'ACCOUNTANT'], async ({ id }) => {
