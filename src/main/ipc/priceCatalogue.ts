...
export function registerPriceCatalogueHandlers(): void {
  ...
  handle<UpsertPricePayload, PriceCatalogue>(
    'priceCatalogue:upsert',
    ['ADMIN'],
    async ({ brick_type, unit_price, is_active }) => {
    ...
    { resource: 'price_catalogue', action: 'CREATE' }
  )

  handle<DeletePricePayload, { deleted: boolean }>(
    'priceCatalogue:delete',
    ['ADMIN'],
    async ({ id }) => {
    ...
    { resource: 'price_catalogue', action: 'DELETE' }
  )
}
